...
int main(int argc, char **argv) {
	...
	auto problem_def = Erf2D5Problem(Nx, Nx, Ny, t_steps, dt, t_shift);
	
	cout << problem_def.Nx1 << " " << problem_def.Nx2 << " " << problem_def.t_steps << " " << problem_def.dt << " " << problem_def.t_shift << endl;
	
	Solver problem = Solver<RefineableQUnsteadyHeatElement<2, X_ORDER>, Erf2D5Problem>(problem_def, info);
