...
// classical stefan problem (liquid at melting point)
...
void get_exact_u(const double &t, const Vector<double> &x, Vector<double> &u) {
	...
	if (x[0] < h) {
		u[0] = 1 - erf(0.5 * x[0]/sqrt(t)) / erf(0.5 * h / sqrt(t));
		u[1] = 1 - erf(0.5 * x[0]/sqrt(t)) / erf(sqrt(0.25 * D[2]));
	} else {
		u[0] = T_l;
		u[1] = T_l;
	}
...
class Erf2DProblem : public Problem {
	private:
		...
		Erf2DProblem(
			...
			geometry = new FreeBoundaryElement(xs[0], xs[1], xs[2], ys[0], ys[1], L / Cp[0], time_stepper_pt());
