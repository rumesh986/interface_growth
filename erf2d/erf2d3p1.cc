...
template<class EL>
class Erf2D3P1Problem : public Problem {
	private:
	...
	public:
		...
		void set_initial_conditions() {
			...
			for (int t = time_stepper_pt()->nprev_values(); t>= 0; t--) {
				...
				printf("[% 2d] Set initial condition at t=%8.6f\n", t, cur_t);
			}

			printf("finished setting initial conditions\n");

			time_pt()->time() = t_shift;
			printf("shifted time again \n");
...
int main(int argc, char *argv[]) {
	...
	auto problem = Erf2D3P1Problem<QUnsteadyHeatElement<2, X_ORDER>>(Nx, Ny, t_steps, dt, t_shift, info);
