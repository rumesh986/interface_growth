...
template<class EL>
class OneDimStefanProblem : public Problem {
	private:
	...
	public:
		OneDimStefanProblem() {
			...
			unsigned int interface_boundary_index = bulk_mesh_pt->free_boundary_index();
		...
		void set_doc_info(DocInfo input) {
			info = input;
		}
...
int main(int argc, char **argv) {
	...
	auto problem = OneDimStefanProblem<QUnsteadyHeatElement<2, X_ORDER>>();

	problem.initialise_dt(params::dt);
	problem.set_doc_info(info);
