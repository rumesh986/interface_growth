...
// uses exponential profile for infinite domain, based on Langer work
...
#include "unsteady_heat_moving.h"
...
namespace params {
	...
	void get_exact_u(const double &t, const Vector<double> &x, double &u) {
		...
		if (x[0] < h) {
			u = 0.0 - gamma * _k * _k * eps * exp(_k * z);
		} else {
			u = exp(-(v/D) * z) - 1.0 + ((v / D)  - gamma * _k * _k) * eps * exp(-_k * z);
		}
	}
...
class TwoDimStefanProblem : public Problem {
	...
	public:
		TwoDimStefanProblem() {
			...
			bulk_mesh_pt = new TwoPhaseFreeBoundarySpineMesh<SpineElement<EL>>(params::nxs, params::ny, params::xs, params::ys, params::gamma, true, time_stepper_pt());
		...
		void actions_after_newton_solve() {
			// printf("v=%8.6f dhdts: ", params::v);
			// for (unsigned int i = 0; i < geometry->ngeom_data(); i++) {
			// 	printf("%u=%8.6f ", i,  geometry->geom_data_pt(i)->time_stepper_pt()->time_derivative(1, geometry->geom_data_pt(i), 0));
			// }
			// printf("\n");
		};
		...
		double get_max_interface() {
			double h_max = 0.0;
			for (unsigned int n = 0; n < geometry->ngeom_data(); n++) {
				if (geometry->geom_data_pt(n)->value(0) > h_max) h_max = geometry->geom_data_pt(n)->value(0);
			}
		...
		void doc_step(const unsigned int &timestep, const unsigned int &t = 0) {
			...
			Vector<double> x(2, 0.0), num_u(1, 0.0);
			double ana_u;
			...
			sprintf(fname, "%s/results.dat", info.directory().c_str());
			file = fopen(fname, "a");
			fprintf(file, "%16.14f %16.14f %16.14f %16.14f %16.14f\n", time, tot_error, num_h, params::get_exact_h(time), max_elem_size);
			fclose(file);
			...
			double max_perturbation = 0.0;
			double base = geometry->geom_data_pt(0)->value(0);
			// printf("interface positions: ");
			for (unsigned int i = 0; i < geometry->ngeom_data(); i++) {
				// printf("%16.14f ", geometry->geom_data_pt(i)->value(0));
				if (fabs(geometry->geom_data_pt(i)->value(0) - base) > fabs(max_perturbation)) max_perturbation = geometry->geom_data_pt(i)->value(0) - base;
			} 
			// printf("\n");
			
			// printf("maximum perturbation size: %16.14f from %16.14f\n", max_perturbation, base);
			printf("[%4u] time=%8.6f error=%10.8e iface_err=%10.8e interface=%16.14f expected=%8.6f max_perturb=%16.14f\n", timestep, time, tot_error, fabs(num_h - params::get_exact_h(time)), num_h, params::get_exact_h(time), max_perturbation);

			sprintf(fname, "%s/max_perturbs.dat", info.directory().c_str());
			file = fopen(fname, "a");
			fprintf(file, "%16.14f\n", max_perturbation);
			fclose(file);
			info.number()++;
...
int main(int argc, char **argv) {
	...
	CommandLineArgs::specify_command_line_flag("--v", &params::v);
	...
	CommandLineArgs::specify_command_line_flag("--x0", &params::xs[0]);
	CommandLineArgs::specify_command_line_flag("--x1", &params::xs[1]);
	CommandLineArgs::specify_command_line_flag("--x2", &params::xs[2]);
	CommandLineArgs::specify_command_line_flag("--y0", &params::ys[0]);
	CommandLineArgs::specify_command_line_flag("--y1", &params::ys[1]);
	...
	auto problem = TwoDimStefanProblem<QUnsteadyHeatMovingElement<2, X_ORDER>>();
	...
	for (unsigned int t = 0;t < params::tsteps; t++) {
		...
		double h = problem.get_max_interface();
